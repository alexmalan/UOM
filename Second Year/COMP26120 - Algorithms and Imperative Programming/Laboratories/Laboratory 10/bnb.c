...
int QUIET=1;     // can be set to 1 to suppress output
int tw;
int tv;
...
void branch_and_bound(int *final_sol)
{
  ...
  struc_sol *new_sol = (struc_sol*)malloc (sizeof(struc_sol));
  struc_sol *child_one = (struc_sol*)malloc(sizeof(struc_sol));
  struc_sol *child_two = (struc_sol*)malloc(sizeof(struc_sol));

  int fixed = 0;

  frac_bound(new_sol, fixed);

  int current_best = new_sol->val;
  insert(*new_sol);

  while(QueueSize != 0 && new_sol->bound > current_best)
  {
    *new_sol = removeMax();
    copy_array(new_sol->solution_vec, child_one->solution_vec);
    copy_array(new_sol->solution_vec, child_two->solution_vec);
    fixed = new_sol->fixed;

    child_one->solution_vec[fixed+1] = 1;
    child_two->solution_vec[fixed+1] = 0;

    child_one->fixed = fixed+1;
    child_two->fixed = fixed+1;

    int k = check_evaluate_and_print_sol(child_one->solution_vec, &tv, &tw);
    if(k == 0)
    {
      frac_bound(child_one, child_one->fixed);
      if(child_one->val > current_best)
        {
          current_best = child_one -> val;
          copy_array(child_one -> solution_vec, final_sol);
        }
        insert(*child_one);
    }
    ...
    int k = check_evaluate_and_print_sol(child_two->solution_vec,&tv, &tw);
    if(k == 0)
    {
      frac_bound(child_two, child_two->fixed);
      if(child_two->val > current_best)
      {
        current_best = child_two->val;
        copy_array(child_two->solution_vec, final_sol);
      }
      insert(*child_two);
    }
